...
const EvaluationView = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if evaluation exists via submission status
        if (['evaluated', 'returned'].includes(res.submission.status)) {
          // Fetch evaluations for this assignment to find the matching one
          const { getEvaluationsByAssignmentApi } = await import('../../api/evaluation.api.js');
          const evalRes = await getEvaluationsByAssignmentApi(res.submission.assignment_id);
  ...
  const handleGrade = async (data) => {
    try {
      ...
      throw err; // keeps GradeForm in submitting state until resolved
    }
  };
  ...
  if (error || !submission) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        <p className="text-sm text-red-600 font-medium">{error || 'Submission not found'}</p>
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6 gap-4">
        <div>
          <h1 className="text-xl font-semibold text-gray-950">{submission.assignment_title}</h1>
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="col-span-2 space-y-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-green-100 flex items-center
                  justify-center text-green-700 text-sm font-semibold">
                  {submission.student_name?.charAt(0).toUpperCase()}
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-950">{submission.student_name}</p>
              ...
              <div className="grid grid-cols-2 gap-3 mt-3">
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">File size</p>
                  <p className="text-sm font-medium text-gray-950 mt-1">
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  ...
                  <p className="text-sm font-medium text-gray-950 mt-1">{submission.max_marks}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3">
              ...
              <button onClick={handleDownload}
                className="w-full flex items-center justify-center gap-2 border border-gray-200
                  ...
                  hover:bg-gray-50 transition-colors mt-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* Current evaluation display — shown if graded */}
          {evaluation && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-green-800">Current evaluation</h2>
                <PlagiarismBadge score={evaluation.plagiarism_score} />
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-green-700">{evaluation.marks_obtained}</span>
                <span className="text-base text-green-600">/ {submission.max_marks}</span>
              ...
              {!isReturned && (
                <button
                  ...
                  disabled={returning}
                  className="mt-4 w-full border border-green-300 text-green-700 text-sm
                    font-medium rounded-lg px-4 py-2 hover:bg-green-100 transition-colors
